...
export class RegisterCustomerComponent implements OnInit {
  ...
  register(){
    if(this.password != this.passwordConf){
      ...
      alert("Lozinka mora da sadrzi veliko slovo, broj i specijalni karakter.")
    } else if(this.usedUsernames.includes(this.username)){
      alert("Korisnicko ime je vec iskorisceno.")
    } else if(this.usedEmails.includes(this.email)){
      alert("Adresa e-poste je vec iskoriscena.")
    } else {
      this.registerService.sendRequest(this.username, this.password, this.name, this.surname, this.address, this.phone, this.email, this.imageSrc).subscribe(()=>{
        alert("Poslat zahtev.");
        this.router.navigate(['/'])
        this.name = this.surname = this.username = this.password = this.passwordConf = this.address = this.phone = this.email = this.imageSrc = "";
      });
    }
  }
  ...
  checkPass(){
    ...
    let firstCode = this.password.charCodeAt(0);
      ...
      for(let i = 0; i < this.password.length; i++){
        ...
        if(code > 96 && code < 123){
          if(i != this.password.length - 1){
            let codeNext = this.password.charCodeAt(i+1);
            if(code == codeNext){
              neighborChars = false;
            }
          }
  ...
  getAllUsers() {
    this.loginService.getAllUsers().subscribe((data: User[]) => {
      ...
      for(let u of this.users){
        this.usedEmails.push(u.mejl);
        this.usedUsernames.push(u.korisnicko_ime);
      }
    })
  }

  getAllRequests() {
    this.loginService.getAllRequests().subscribe((data: Request[]) => {
      this.requests = data;
      for(let r of this.requests){
        if(r.status == "odbijen"){
          this.usedEmails.push(r.mejl);
          this.usedUsernames.push(r.korisnicko_ime);
        }
      }
    })
  }
